...
def depth_1(sample_indices):
    ...
    return parent_node
def ops_per_depth_1():
    return [len(UNARY_OPS)]


def test_config(sample_indices):
    leaf1 = Node(operation_type="leaf", leaf_idx=0, name="leaf1", operation=None)
    leaf2 = Node(operation_type="leaf", leaf_idx=1, name="leaf2", operation=None)

    l2_branch1 = Node(operation_type="unary", operation=UNARY_OPS[sample_indices[3]], left=leaf1, name="l2_branch1")
    l2_branch2 = Node(operation_type="unary", operation=UNARY_OPS[sample_indices[2]], left=leaf2, name="l2_branch2")

    l1_node = Node(operation_type="binary", operation=BINARY_OPS[sample_indices[1]], left=l2_branch1, right=l2_branch2, name="parent_node")

    parent_node = Node(operation_type="unary", operation=UNARY_OPS[sample_indices[0]], left=l1_node, name="parent_node")

    return parent_node

def ops_per_test_config():
    return [len(UNARY_OPS), len(BINARY_OPS), len(UNARY_OPS), len(UNARY_OPS)]
...
depth_1_tree_config = TreeConfig(tree_func=depth_1, ops_per_node_func=ops_per_depth_1, num_leaves=1)
...
TREE_CONFIGS = {
    "depth_1_tree_config": depth_1_tree_config,
