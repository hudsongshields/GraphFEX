...
def main():
    ...
    parser.add_argument('--output', type=str, default="NumericalExperiments/Lorenz/recovered_scindy")
    args = parser.parse_args()

    timesteps=5000
    dt = 0.01
    adj_matrix = make_adjacency(100, 3)
    timeseries, t_derivs = make_data(num_samples=timesteps, adjacency=adj_matrix, snr=args.snr, coupling=0.8, smoothing=True)
    ...
    self_library = (ps.PolynomialLibrary(degree=4, include_bias=True)) # + ps.FourierLibrary(n_frequencies=1) + sigmoid_library)
    neighbor_library = (ps.PolynomialLibrary(degree=3, include_bias=True)) # + ps.FourierLibrary(n_frequencies=1) + sigmoid_library)
    ...
    optim_name = type(model.optimizer).__name__
    if args.output is not None:
        log_path = f"{args.output}_snr{args.snr}.txt"
        with open(log_path, "w") as f:
            model.print(file=f)
            f.write(f"{optim_name} iterations: {n_iter}/{model.optimizer.max_iter}\n")
