...
def main():
    ...
    test_lines = [
        ...
        "H0: mean(ground_truth_rewards) <= mean(random_sequence_rewards)",
        "H1: mean(ground_truth_rewards) > mean(random_sequence_rewards)",
        f"mean_ground_truth: {np.mean(random_ground_truth_rewards_np):.6f}",
        f"mean_random_sequence: {np.mean(random_init_rewards_np):.6f}",
    ...
    plt.legend([f"Average Reward (Random Sequences): {np.mean(random_init_rewards):.4f}", f"Average Reward (Ground Truth): {np.mean(random_ground_truth_rewards):.4f}"])
